...
const request = async (url, options = {}) => {
  try {
    ...
    if (error.name === 'TypeError' && error.message.includes('fetch')) {
      console.warn('Backend server is offline. Running in Mock Standalone Mode.');
...
const handleMockFallback = async (url, options) => {
  ...
  if (url === '/videos/process') {
    ...
    if (existingIndex > -1) {
      ...
      const title = videoId === 'Ke90Tje7VS0' 
    ...
    localStorage.setItem(`mock_materials_${videoId}`, JSON.stringify(studyMaterials.studyMaterial));
    localStorage.setItem(`mock_transcript_${videoId}`, JSON.stringify(studyMaterials.video.transcript));
  ...
  if (url.startsWith('/videos?q=') || url === '/videos') {
    ...
    if (query) {
      ...
      result = mockVideos.filter(v => searchRegex.test(v.title) || searchRegex.test(v.channelTitle));
    }
  ...
  if (url.startsWith('/videos/')) {
    ...
    const video = mockVideos.find(v => v._id === id || v.videoId === id || `mock-video-${v.videoId}` === id);
    if (!video) throw new Error('Video not found');

    const videoId = video.videoId;
    let studyMaterial = JSON.parse(localStorage.getItem(`mock_materials_${videoId}`));
    let transcript = JSON.parse(localStorage.getItem(`mock_transcript_${videoId}`));
    ...
    if (!studyMaterial || !transcript) {
      ...
      localStorage.setItem(`mock_materials_${videoId}`, JSON.stringify(studyMaterial));
      localStorage.setItem(`mock_transcript_${videoId}`, JSON.stringify(transcript));
    }
    ...
    return {
      ...
      userProgress: {
        ...
        chatHistory: JSON.parse(localStorage.getItem(`mock_chat_${videoId}`)) || [],
      }
    };
  }
  ...
  if (url.startsWith('/videos/') && options.method === 'DELETE') {
    const id = url.split('/')[2];
    const filtered = mockVideos.filter(v => v._id !== id && v.videoId !== id && `mock-video-${v.videoId}` !== id);
  ...
  if (url.endsWith('/progress') && options.method === 'PUT') {
    ...
    const { type, flashcardId, score, totalQuestions, notesCompleted } = JSON.parse(options.body);

    const videoIndex = mockVideos.findIndex(v => v._id === id || v.videoId === id || `mock-video-${v.videoId}` === id);
...
4. **Durability**: Committed data survives system crashes.`;
    } else if (cleanMsg.includes('normalize') || cleanMsg.includes('database')) {
    ...
    } else if (cleanMsg.includes('hi') || cleanMsg.includes('hello') || cleanMsg.includes('hey')) {
...
export const videosAPI = {
  process: (url) => request('/videos/process', {
    method: 'POST',
    body: JSON.stringify({ url }),
  }),
  getAll: (searchQuery = '') => {
    const url = searchQuery ? `/videos?q=${encodeURIComponent(searchQuery)}` : '/videos';
    return request(url);
  },
  ...
  updateProgress: (id, progressData) => request(`/videos/${id}/progress`, {
    method: 'PUT',
    body: JSON.stringify(progressData),
  }),
  chat: (id, message) => request(`/videos/${id}/chat`, {
    method: 'POST',
